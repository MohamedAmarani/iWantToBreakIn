...
bool TileMap::collisionMoveLeft(const glm::ivec2 &pos, const glm::ivec2 &size, int b)
{
	...
	for (int y = y0; y <= y1; y++)
	{
		if (map[(y + (offset * 32))*mapSize.x + x] != 0) {
			if(b == 1 && (map[(y + (offset * 32))*mapSize.x + x] != 1 && map[(y + (offset * 32))*mapSize.x + x] != 2 && map[(y + (offset * 32))*mapSize.x + x] != 3))
...
int TileMap::collisionMoveDownBall(const glm::ivec2 &pos, const glm::ivec2 &size, int b)
{
